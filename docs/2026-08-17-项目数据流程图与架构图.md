...
> 状态：v1.0 与 `feat/group-chat-implementation` 分支当前实现一致；v1.1 新增 4.11-4.13「加入房间三入口」规划章节（标注「规划中」，待实现）
...
## 4.11 加入房间三入口（v1.1 规划中）

> 本节为已确认设计的规划章节，落地后移除「规划中」标注并同步第 5 节接口清单。
...
规划代码位置：`client/src/renderer/src/api/rooms.ts`（新增 joinRoomApi）、`client/src/renderer/src/stores/chat.ts`（新增 joinRoom action）、`client/src/renderer/src/components/RoomList.tsx`（加入弹窗）、`client/src/renderer/src/utils/url.ts`（链接解析）。
...
sequenceDiagram
    ...
    UI->>PRE: window.chatAPI.copyInviteLink(roomId)<br/>（或前端 clipboard 直接复制）
...
规划代码位置：`client/src/main/index.ts`（协议注册 + open-url/second-instance）、`client/src/preload/index.ts`（copyInviteLink / onInviteLink 白名单通道）、`client/src/renderer/src/pages/Chat.tsx`（监听后打开弹窗）。
...
规划代码位置：`server/src/services/room.service.ts`（新增 invite）、`server/src/controllers/room.controller.ts` + `server/src/routes/rooms.ts`（POST invite）、`server/src/repositories/room.repo.ts`（findByUsername / addMember）、`server/src/sockets/handlers.ts`（emit room:invited）、`client/src/renderer/src/api/rooms.ts`、`client/src/renderer/src/stores/chat.ts`、`client/src/renderer/src/components/MembersPanel.tsx`、`client/src/renderer/src/services/socket.ts`（onRoomInvited）。
...
| POST | `/api/v1/rooms/:roomId/invite` | Bearer | 邀请用户入房 `{ username }`（v1.1 规划中，见 4.11.3） |
...
| S→C | `room:invited` | `{ roomId }` | 被邀请者实时刷新房间列表（v1.1 规划中，见 4.11.3） |

握手：`io.use` 从 `socket.handshake.auth.token` 校验 JWT，失败拒绝连接。

### 5.3 preload/IPC 深链通道（v1.1 规划中，见 4.11.2）

| 通道 | 方向 | 说明 |
|---|---|---|
| `window.chatAPI.copyInviteLink(roomId)` | 渲染→主进程 | 复制 `groupchat://join?roomId=xxx` 邀请链接 |
...
| 邀请成员按钮 | MembersPanel 的"邀请"按钮暂无逻辑 | 4.5 附近，`components/MembersPanel.tsx` |

> 后续若完善上述任一项，必须沿用本文档对应流程的通道与分层，不另起炉灶。
>
> v1.1 规划：4.11.1-4.11.3「加入房间三入口」实现后，本表中「邀请成员按钮」一项将同步更新为已实现。
